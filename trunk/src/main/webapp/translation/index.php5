...
function getTranslationFromGoogle($lang, $textToTranslate, $encoding, $example) {
  ...
  $translatedText = file_get_contents('http://translate.google.com/translate_t', FILE_TEXT, $context);
  ...
  $result = utf16encode($result);
  if($displayExample) $result='['.$example.']'.$result;
  return $result;
}
function utf16encode($string) {
  $codes = split(" ",preg_replace('/%/', ' ',url_encode($string)));
  $result = "";
  $compte = 0;
  $n = 0;
  foreach($codes as $code) {
    if($code!='') {
      $b = hexdec($code);
      if($compte==0) {
        if(0 <= $b && $b <= 0x7F) { $result .= '&#x'.dechex($b).';'; } // 0xxxxxxx
        else if (0xC0 <= $b && $b <= 0xDF) { $compte = 1; $n = $b & 0x1F; } // 110xxxxx
        else if (0xE0 <= $b && $b <= 0xEF) { $compte = 2; $n = $b & 0xF; } // 1110xxxx
        else if (0xF0 <= $b && $b <= 0xF7) { $compte = 3; $n = $b & 0x7; } // 11110xxx
        else { $result .= '!erreur0 ' . dechex($b) . '! '; }
      }
      else if ($compte==1) {
        if ($b < 0x80 || $b > 0xBF) { $result .= '!erreur1 ' . dechex($b) . '! '; }
        $compte--;
        $result .= '&#x'.dechex(($n << 6) | ($b-0x80)) . ';';
        $n = 0;
      }
      else if ($compte==2 || $compte==3) {
        if ($b < 0x80 || $b > 0xBF) { $result .= '!erreur2 ' . dechex($b) . '! '; }
        $n = ($n << 6) | ($b-0x80);
        $compte--;
      }
    }
...
function getLanguages() {
  ...
  $languages = array(
    ...
    "uk"=>"Ukrainian,ua.gif,KOI8-R,&#x441;&#x43f;&#x430;&#x441;&#x438;&#x431;&#x43e;",
